...
- `DyadicDataAnalysis.Rmd` is the primary slide source; `pdf_version.Rmd` is the PDF variant.
...
- Rendered artifacts live in `DyadicDataAnalysis.html`, `pdf_version.pdf`, and their `*_files/` directories.
...
- `rmarkdown::render("DyadicDataAnalysis.Rmd")` rebuilds the HTML slides.
- `rmarkdown::render("pdf_version.Rmd")` rebuilds the PDF output.
...
- PRs should summarize changes, list any regenerated outputs (`DyadicDataAnalysis.html`, `pdf_version.pdf`), and call out cache changes if they affect results.
