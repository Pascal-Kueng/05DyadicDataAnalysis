...
idiff_example <- df_long_dim |>
  select(
    ...
    cwp_dyad_mean = provided_support_cwp_dyad_mean,
    cwp_dev = provided_support_cwp_dev
  )
...
substantive meaning. The tutorial helper removes the leading dot while retaining
the package's `cwp_actor`, `cbp_partner`, and `cwp_dyad_mean` ordering, and
shortens `within_dyad_dev` to `cwp_dev`.
...
The common and `idiff` components are rotated to member-level quantities in the
