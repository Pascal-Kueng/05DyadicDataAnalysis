---
title: "Distinguishable Dyads: Model Specification and Output"
author: "Pascal Küng"
...
[@bolgerIntensiveLongitudinalMethods2013; @kenny2006dyadic]:
...
```{r distinguishable-data-preparation}
dist_raw <- tibble(
  coupleID = rep(31:32, each = 6),
  userID = rep(c("31_1", "31_2", "32_1", "32_2"), each = 3),
  diaryday = rep(0:2, 4),
  gender = rep(c("female", "male", "female", "male"), each = 3),
...
dist_example <- df_long_apim |>
  select(
    coupleID, userID, diaryday, gender, is_female, is_male,
    cwp_actor = provided_support_cwp_actor,
    cwp_partner = provided_support_cwp_partner,
    cbp_actor = provided_support_cbp_actor,
    cbp_partner = provided_support_cbp_partner
  )
...
`prepare_dyad_data()` creates the role indicators and the within-person and
between-person actor-partner predictors. The tutorial helper removes the
leading dot, yielding names such as `provided_support_cwp_actor` and
