...
```{=html}
    ...
    <div class="sd2-panel">
      ...
      <div class="sd2-group">
        ...
        <div class="sd2-control sd2-control-actor">
          <label for="sd2-actor">Actor predictor, x<sub>actor</sub></label>
        ...
        <div class="sd2-control sd2-control-partner">
          <label for="sd2-partner">Partner predictor, x<sub>partner</sub></label>
      ...
      <div class="sd2-group">
        <div class="sd2-group-title">
          ...
          DIM: sum and difference coordinates
        </div>
        <div class="sd2-control sd2-control-cbc">
          <label for="sd2-cbc">Couple mean, x<sub>cbc</sub></label>
          <output id="sd2-out-cbc" style="color:var(--cbc)">0.0</output>
          <input id="sd2-cbc" type="range" min="-5" max="5" step="0.05" value="0">
        </div>
        <div class="sd2-control sd2-control-cwc">
          <label for="sd2-cwc">Couple deviation, x<sub>cwc</sub></label>
          <output id="sd2-out-cwc" style="color:var(--cwc)">0.0</output>
          <input id="sd2-cwc" type="range" min="-5" max="5" step="0.05" value="0">
        </div>
      </div>

      <div class="sd2-relations">
        <div>x<sub>cbc</sub> = (x<sub>actor</sub> + x<sub>partner</sub>) / 2</div>
        <div>x<sub>cwc</sub> = (x<sub>actor</sub> - x<sub>partner</sub>) / 2</div>
      </div>
    </div>
    ...
    <main class="sd2-main">
      ...
      <div class="sd2-grid-wrap">
        ...
        <div class="sd2-explain">
          <div>
            <strong>Between-couple effect: the sum.</strong>
            If both partners increase by 1, the couple mean increases by 1 and the deviation stays 0.
            <button class="sd2-demo" id="sd2-demo-sum" type="button">Demonstrate</button>
            <div class="sd2-mini-eq">b<sub>cbc</sub> = b<sub>actor</sub> + b<sub>partner</sub></div>
          </div>
          <div>
            <strong>Within-couple effect: the difference.</strong>
            If the actor increases by 1 and the partner decreases by 1, the couple mean stays 0 and the deviation increases by 1.
            <button class="sd2-demo" id="sd2-demo-diff" type="button">Demonstrate</button>
            <div class="sd2-mini-eq">b<sub>cwc</sub> = b<sub>actor</sub> − b<sub>partner</sub></div>
          </div>
          <div>
  ...
  (function(){
    ...
    const BASE_MIN = -5, BASE_MAX = 5, PLOT_DOMAIN = 10;
    ...
    function syncSlider(slider, value){
      const lo = Math.min(BASE_MIN, Math.floor(value));
      const hi = Math.max(BASE_MAX, Math.ceil(value));
      slider.min = lo;
      slider.max = hi;
      slider.value = value;
    }
    function syncInputs(){
      syncSlider(actor, state.a);
      syncSlider(partner, state.p);
      syncSlider(cbc, state.m);
      syncSlider(cwc, state.w);
    }

    function setFromAP(a, p){
      state.a = r1(a);
      state.p = r1(p);
      state.m = r2((state.a + state.p) / 2);
      state.w = r2((state.a - state.p) / 2);
      syncInputs();
      render();
    }
    function setFromDIM(m, w){
      state.m = r2(m);
      state.w = r2(w);
      state.a = r2(state.m + state.w);
      state.p = r2(state.m - state.w);
      syncInputs();
      render();
    }
    function moveActor(){
      setFromAP(actor.value, state.p);
    }
    function movePartner(){
      setFromAP(state.a, partner.value);
    }
    function moveCbc(){
      setFromDIM(cbc.value, state.w);
    }
    function moveCwc(){
      setFromDIM(state.m, cwc.value);
    }
    ...
    function render(){
      ...
      eqA.innerHTML = `Ŷ = ${fmt(b0)} + ${fmt(bA)} × <span style="color:${c.actor}">${fmt(a)}</span> + ${fmt(bP)} × <span style="color:${c.partner}">${fmt(p)}</span> = <strong>${fmt(yA)}</strong>`;
      eqD.innerHTML = `Ŷ = ${fmt(b0)} + ${fmt(bCbc)} × <span style="color:${c.cbc}">${fmt(m)}</span> + ${fmt(bCwc)} × <span style="color:${c.cwc}">${fmt(w)}</span> = <strong>${fmt(yD)}</strong>`;
      eqC.innerHTML = `b<sub>cbc</sub> = ${fmt(bA)} + ${fmt(bP)} = ${fmt(bCbc)}; &nbsp; b<sub>cwc</sub> = ${fmt(bA)} − ${fmt(bP)} = ${fmt(bCwc)}`;
    ...
    function drawGrid(){
      ...
      el('sd2-axis-labels').innerHTML = `
        <text x="286" y="154" fill="${c.actor}" font-size="9" font-weight="800" text-anchor="end">x<tspan baseline-shift="sub" font-size="7">actor</tspan></text>
        <text x="160" y="22" fill="${c.partner}" font-size="9" font-weight="800" text-anchor="middle">x<tspan baseline-shift="sub" font-size="7">partner</tspan></text>
        <g transform="translate(${xPix(6.3)},${yPix(6.3)}) rotate(-45)"><text fill="${c.cbc}" font-size="8" font-weight="800" text-anchor="middle">x<tspan baseline-shift="sub" font-size="6">cbc</tspan></text></g>
        <g transform="translate(${xPix(6.3)},${yPix(-6.3)}) rotate(45)"><text fill="${c.cwc}" font-size="8" font-weight="800" text-anchor="middle">x<tspan baseline-shift="sub" font-size="6">cwc</tspan></text></g>`;
    }
    ...
    bindButton('sd2-demo-sum', () => setFromAP(1, 1));
    bindButton('sd2-demo-diff', () => setFromAP(1, -1));
