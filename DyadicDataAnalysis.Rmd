...
Often, this is attempted via: `(1 | coupleID)`.
  ...
  - With the usual Gaussian residual variance still present, the implied partner correlation is positive but not freely estimated: `tau^2 / (tau^2 + sigma^2)`.
...
The gender-coded cross-sectional version does not transfer directly to ILD. For repeated measures, we instead index the homogeneous two-partner residual structure at the couple-day level, as shown later.

## Exchangeable Dyads - Cross-Sectional APIM: brms

In brms, this issue can also be solved with a native residual correlation structure
