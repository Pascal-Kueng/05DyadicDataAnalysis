...
model1_glmmTMB <- glmmTMB(
  satisfaction ~ 
    ...
    # Modelling non-independence by using compound symmetry of the residuals. 
...
Usually DHARMa {!TODO: cite package} is an amazing tool for model diagnostics for complex (generalized) MLM. 
...
formula <- bf(
  satisfaction ~ 
    ...
    # Modelling non-independence by using compound symmetry of the residuals. 
    # Note that we can't use unstructured here, due to a brms limitation.
    # But compound symmetry is exactly equivalent with only two partners. 
    cosy(gr = coupleID),
...
CAREFUL: In contrast to glmmTMB, brms reports sigma on the log scale (see `?brms::family.brmsfit()`). Using `exp(value)` retrieves the simulated SD values well and aligns perfectly with the glmmTMB model. 
...
We need to compute the residual for each partner ourselves:
```{r}
0.72170702 * 1.000000
0.72170702 * 1.554608
...
Note: When comparing to the glmmTMB output, do not forget to exponentiate the two sigmas from brms. 

!TODO: change reporting function to exponentiate from the start for the tables?

## Distinguishable Dyads - L-APIM - AR1 Model 

In glmmTMB, we can try to add an AR1 residual structure.

```{r eval = FALSE}
    
    # Accounting for non-independence between partners' means and trajectories 
    # and effect sensitivities
    (0 + is_male + is_female
      + is_male:diaryday_c + is_female:diaryday_c
    | coupleID) +
    ...
    # Accounting for each persons' autocorrelation. 
...
However, these autocorrelation structures are additive. Additionally, this would imply that both partners in the couple have the exact same AR process. 

**This is not fully in line with the idea of distinguishable dyads!**

This does also not work in brms, as only one residual structure at a time is supported. 

## Workarounds if AR processes are plausible

In brms, a dynamic MLM can account for temporal dependence from both the actor and partner. 

!TODO: briefly describe. 
!TODO: briefly explain why this is risky in glmmTMB due to nickel bias or what it is called, and stationarity assumptions being challenged by random effects? if that is correct. 
!TODO: also mention how this changes interpretation. Show an example implementation

Future covariance structures that improve on this are in the works in glmmTMB.
...
!TODO: this was written without cosy in mind but rather a random effect like (1 | coupleID:day). So this needs adjusting. 
...
truth_tbl <- tibble::tribble(
  ...
  "Residuals", "Residual SD (male) (exp)",   sd(panel$e_it[panel$is_male   == 1]), "exp",
  "Residuals", "Residual SD (female) (exp)", sd(panel$e_it[panel$is_female == 1]), "exp"
)
...
Now, we only have one actor effect and one partner effect. This pools the information from both 
partners within a couple. 

For the random effects: What we want is to constrain both partners variances to be equal. 
We could pool similarly as we do in the fixed effects by doing: `(1 | coupleID)`. 

However, this now does not estimate a correaltion between partners anymore!
