...
AR1 sometimes changes the fixed effects quite a bit in brms. You may always want to check which model 
performs better! Often the same day shock term and the time slopes take care of everything already. 

In this instance, the AR1 model is much worse!
...
# Adding back random slopes

We add back more random slopes (that we were unable to include in the frequentist
frameworks). We compare models again to check if we are overfitting. 
...
The difference between the models is quite small. We could follow the p-value and the principle of parsimony, 
or our theory and the philosophy of the maximal random effects strucutre (e.g., Barr et al., REF). 

Alternatively, we could exclude the smallest random effects and compare models again. 

For more guidance on random effects check out @delrosarioPracticalGuideSpecifying2025. 
...
Equivalence holds on both levels. In the stronger effects (within-person apim matches the within-person DIM almost perfectly), whereas the between-person apim and the between-person dim show slight numeric deviations. This is due to comparing wide posteriors with high uncertainty (weakly identified). 
