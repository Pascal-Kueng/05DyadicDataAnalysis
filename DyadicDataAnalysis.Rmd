...
**Suggested citation:** Küng, P. M. (2026). *Distinguishable and Exchangeable Dyads: Bayesian Multilevel Modelling* (Version 2.0.11) [Report]. Zenodo. [https://doi.org/10.5281/zenodo.17400655](https://doi.org/10.5281/zenodo.17400655)
...
`dyadMLM` marks generated columns with a leading dot. This tutorial drops that
dot and shortens the `within_dyad_dev` suffix to
...
|:---|:---|:---|
| `.is_female` | `is_female` | female member of a female-male dyad |
| `.is_male` | `is_male` | male member of a female-male dyad |
| `.member_contrast_arbitrary` | `idiff` | arbitrary exchangeability contrast |
| `.communication_actor` | `communication_actor` | actor's communication score |
| `.communication_partner` | `communication_partner` | partner's communication score |
| `.communication_gmc_actor` | `communication_gmc_actor` | grand-mean-centered actor score |
| `.communication_gmc_partner` | `communication_gmc_partner` | grand-mean-centered partner score |
| `.communication_dyad_mean_gmc` | `communication_dyad_mean_gmc` | grand-mean-centered dyad mean |
| `.communication_within_dyad_dev` | `communication_dyad_dev` | member's within-dyad deviation |
...
print_apim_preview <- function(df) {
  preview_cols <- c(
    "userID", "coupleID", "gender", "is_male", "is_female", "idiff",
    "satisfaction", "communication_gmc_actor", "communication_gmc_partner"
  )
  df <- df |>
    dplyr::select(dplyr::any_of(preview_cols)) |>
    dplyr::rename(
      gmc_actor = communication_gmc_actor,
      gmc_partner = communication_gmc_partner
    )
  print_df(head(df))
}
...
Exchangeability is a modelling constraint, not a claim that the dyads are truly
exchangeable. To test that constraint, fit both models to the same prepared
observations.
...
## Exchangeable APIM - Covariance Recovery {.smaller}

`recover_exchangeable_covariance()` back-transforms the common and deviation
blocks to the homogeneous member variance and partner correlation:
...
dyadMLM::recover_exchangeable_covariance(
  model_exch_apim_glmmtmb,
  block_pairings = list(
    shared_block = "(1 | coupleID)",
    difference_block = "(0 + idiff | coupleID)",
...
communication_grand_mean <- mean(df_exchangeable_apim$communication)
...
## Constraints vs Prediction {.smaller}

`compare_nested_models()` tests the imposed exchangeability constraints with a
likelihood-ratio comparison:

```{r compare-exch-dist-apim-glmmtmb, echo=FALSE, cache=TRUE, dependson=c('fit-dist-apim-glmmtmb', 'fit-exch-apim-glmmtmb')}
...
The following observation-level PSIS-LOO comparison instead evaluates
conditional predictive performance [@buerknerEfficientLeaveOneOut2021]. It is
not a formal test of distinguishability or prediction of a new dyad.
...
For the DIM parameterization, prepare the raw observations under the
exchangeability assumption.

```{r data-exch-dim-source-preview, echo=FALSE, cache=TRUE, dependson='simulate-cross-sectional-data'}

df_exchangeable_dim <- prepare_dyad_data(
  data = df,
  dyad = coupleID,
  member = userID,
  role = gender,
...
```{r fit-exch-dim-brms, results='hide', message=FALSE, warning=FALSE, cache=TRUE, dependson='data-exch-dim-source-preview'}
