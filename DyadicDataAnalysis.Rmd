...
In glmmTMB, random effects with various covariance structures can be specified. A residual covariance can not be directly specified. 
However, in glmmTMB specifically, we can remove the separate modelling of an independen residual by setting `dispformula = ~ 0`. 
...
In brms, we can not use the glmmTMB trick and disable the internal modelling of the residual variance. Instead, we must rely on brms provided residual covariance structures. We can achieve the same model via:
...
```
Note that we could also add `us` before the first random effect term, as this is equivalent. But because the lowest level random effects block is the one that represents the residual covariance strucutre, we only use it there. 
...
summary(model_dist_apim_long_complex)
```
...
- For the fixed effects, we can pool the intercepts `is_male` and `is_female` by simly
...
- For the residual, we want is to constrain both partners variances to be equal. 
Often, this is attempted via: `(1 | coupleID)`. 
  - This fixes both partners' residual variances to be equal (pooled)
  - However, it does **not** allow for a correaltion between partners anymore! The correlation between partners is implied to be perfect (1). 
...
model1_glmmTMB <- glmmTMB(
  satisfaction ~ 
    ...
    # homogenous compound symmetry structure constrains the variances to be equal.
...
formula <- bf(
  satisfaction ~ 1 + 
    ...
    # To achieve a homogenous compound symmetry covariance structure we use
...
Leave-one-out (loo) cross-validation for model comparison (even if not nested). !TODO: even if not nested is it meaningful?
...
model_apim_ind_long_glmmTMB <- glmmTMB(
  closeness ~ 1 + 
    ...
    # To model this in glmmTMB, we can either do: 
    
    # Option 1 (as before): 
    homcs(0 + is_partner1 + is_partner2 | coupleID:diaryday)
    
    # Option 2 (in line with the Idiff approach above, 
    # but would need to be rotated back for clean interpretation):
    # un(1 | coupleID) + un(0 + Idiff | coupleID)
...
summary(model_apim_ind_long_glmmTMB)
```

The model does not converge properly. This means, we would have to simplify the random
Effects structure. Instead, we use brms.
