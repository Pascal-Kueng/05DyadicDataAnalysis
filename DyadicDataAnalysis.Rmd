...
Therefore, we can achieve the same model as before via:
...
`unstr()` is used here instead of `cosy()` because `cosy()` currently restricts correlations to nonnegative values in brms. The unstructured residual correlation can be negative and is therefore the closer analogue to the `glmmTMB` `us(...)` formulation.
...
```{r fit-dist-apim-brms-poisson, results='hide', message=FALSE, warning=FALSE}
#| code-line-numbers: "1-13|15-21|23-28|30-41"
...
```{r fit-dist-apim-glmmtmb-poisson}
...
# Distinguishable Dyads
### Intensive Longitudinal APIM (L-APIM)
...
::: {.brms-prior-note}
Because this distinguishable model estimates partner-specific residual SDs via `sigma ~ 0 + is_male + is_female`, the sigma priors target brms distributional coefficients: `class = "b", dpar = "sigma"`.
:::
