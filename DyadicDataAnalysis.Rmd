...
In glmmTMB, random effects with various covariance structures can be specified. A residual covariance cannot be directly specified. 
However, in glmmTMB specifically, we can remove the separate modelling of an independent residual by setting `dispformula = ~ 0`. 

This forces all residual variance into the random effect we specify. The random effects block now becomes the residual, giving us enormous control over the covariance structure of the residuals. 
...
```{r results-dist-apim-glmmtmb, echo=FALSE}
...
Usually DHARMa [@R-DHARMa] is an amazing tool for simulation-based residual diagnostics for complex (generalized) MLMs. 
However, when we remove the native residuals `dispformula = ~ 0` the residual is, in practice, set to a tiny constant. 

This confuses model diagnostic packages! I am working on a fix. 
...
```{r results-dist-apim-long-glmmtmb, echo=FALSE}
...
```{r results-exch-apim-glmmtmb, echo=FALSE}
...
```{r results-exch-long-apim-glmmtmb, echo=FALSE}
