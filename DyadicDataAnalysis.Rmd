...
abstract: >
  ...
  random-effects structures, heterogeneous residual variances, and compound-symmetry
  residuals. We illustrate diagnostics and model
...
csl: Embed/apa.csl
nocite: |
  @R-brms, @R-easystats, @R-DHARMa, @R-tidyverse, @R-cmdstanr, @R-loo, @R-rstan, @R-MASS, @R-nlme
...
**Suggested citation:** Küng, P. M. (2026). *Distinguishable and Exchangeable Dyads: Bayesian Multilevel Modelling* (Version 2.0.13) [Report]. Zenodo. [https://doi.org/10.5281/zenodo.17400655](https://doi.org/10.5281/zenodo.17400655)
...
# -- 2a: SEPARATE variance components for each group --
...
# --- Residuals with compound symmetry (per dyad) & sex-specific SDs -------
...
retain the raw actor and partner scores, so their intercepts refer to a
communication score of 0 rather than the sample mean.
...
by the random-effect block [@glmmTMB2017]. DHARMa's default conditional simulation
...
#| code-line-numbers: "1-13|15-21|23-27|29-40"
...
#| code-line-numbers: "1-13|15-19|21-26|27-39"
...
Check Rhats and Effective Sample Sizes (ESS_tail and ESS_bulk) directly from the brms summary: Rhat should be below 1.01 and both ESS above 400. Some longitudinal example fits below fall short of this (Rhat up to 1.05); for real analyses, run more iterations. Additionally you can (using Model B as an example):
...
print_df(
  print_couple_preview_for_output(
    ...
    couple_id = "31",
    pdf_cols = c(
      "userID", "diaryday", "diaryday_c", ".is_male", ".is_female",
...
model_dist_apim_long_glmmtmb <- glmmTMB(
  ...
  # no separate residual term: the us() block above models the residuals
  , dispformula = ~ 0
  , data = df_long_apim
)
```
...
print_df(
  ...
  rows_to_pack = list(
    ...
    'Random Effects' = c(13, 22),
    'Residual Structure' = c(23, 25)
    ),
  scroll_height = '55vh'
  )
...
separable partner-by-time residual process [@glmmTMB2017].
...
print_df(
  ...
  rows_to_pack = list(
    'Fixed Effects' = c(1,12),
    'Random Effects' = c(13, 48),
    'Residual Structure' = c(49, 51)
    ),
  scroll_height = '55vh'
  )
...
[@sivula2025]. If performance were similar, theory, identifiability, and
...
map_fixed <- tibble::tribble(
  ~param_regex,                              ~label,
  "^b_\\.is_male$",                                      "Intercept (male)",
  "^b_\\.is_female$",                                    "Intercept (female)",
  "^b_\\.is_male:diaryday_c$",                           "Time slope (male)",
  "^b_\\.is_female:diaryday_c$",                         "Time slope (female)",
  "^b_\\.is_male:\\.provided_support_cbp_actor$",       "Actor BP (male)",
  "^b_\\.is_male:\\.provided_support_cbp_partner$",     "Partner BP (male)",
  "^b_\\.is_female:\\.provided_support_cbp_actor$",     "Actor BP (female)",
  "^b_\\.is_female:\\.provided_support_cbp_partner$",   "Partner BP (female)",
  "^b_\\.is_male:\\.provided_support_cwp_actor$",       "Actor WP (male)",
  "^b_\\.is_male:\\.provided_support_cwp_partner$",     "Partner WP (male)",
  "^b_\\.is_female:\\.provided_support_cwp_actor$",     "Actor WP (female)",
  "^b_\\.is_female:\\.provided_support_cwp_partner$",   "Partner WP (female)"
)
...
#   b_sigma_.is_male, b_sigma_.is_female  (on log scale)
draws <- as_draws_df(fit)
resid_map <- tribble(
  ~param_draw,            ~label,
  "b_sigma_.is_male",      "Residual SD (male) (exp)",
  "b_sigma_.is_female",    "Residual SD (female) (exp)"
)
...
variance of the independent member deviation. With
`.member_contrast_arbitrary = -1/+1`, the
...
comparison_exch_dist <- dyadMLM::compare_nested_models(
  model_dist_apim_glmmtmb,
  model_exch_apim_glmmtmb
)

if (knitr::is_latex_output()) {
  comparison_table <- comparison_exch_dist
  class(comparison_table) <- c("anova", "data.frame")
  print(comparison_table)
  cat(
    "\n",
    paste(strwrap(attr(comparison_exch_dist, "conclusion"), width = 82),
          collapse = "\n"),
    "\n",
    sep = ""
  )
} else {
  print(comparison_exch_dist)
}
```
...
its uncertainty [@loo2017a; @sivula2025].
...
```{=html}
  ...
  <div id="model-coefs"
       ...
       data-dim-cw ="`r if (knitr::is_html_output()) round(fixef(model_exch_dim)['.communication_within_dyad_dev','Estimate'], 2) else ''`"></div>
...
(function(){
  ...
  function render(){
    const A=+actor.value, P=+partner.value, M=(A+P)/2, W=(A-P)/2;
  ...
  function updateOutputs(){
    ...
    outM.value=((+actor.value + +partner.value)/2).toFixed(2);
    outW.value=((+actor.value - +partner.value)/2).toFixed(2);
  }
...
```{=html}
  ...
  (function(){
    ...
    function render(){
      const a = Number(actor.value), p = Number(partner.value), m = (a + p) / 2, w = (a - p) / 2;
...
print_df(
  print_couple_preview_for_output(
    df_long_dim,
    couple_id = "31",
    days = if (knitr::is_latex_output()) 0:2 else 0:3,
    pdf_cols = c(
      "userID", ".member_contrast_arbitrary", "diaryday", "diaryday_c",
      "closeness", ".provided_support_cwp_actor",
      ".provided_support_cwp_partner"
    )
  ),
...
The generated `.provided_support_cwp_dyad_mean` and `.provided_support_cwp_within_dyad_dev`
columns form the within-person DIM. The corresponding
`.provided_support_cbp_dyad_mean` and `.provided_support_cbp_within_dyad_dev` columns form the
between-person DIM [@bolgerIntensiveLongitudinalMethods2013].

Caveat: these simulated data contain gender differences, which exchangeable
models ignore. The member-level variances and correlations reported below
absorb these differences, so they do not match the simulated values. The
example only illustrates the workflow.

## Exchangeable Dyads - L-DIM/L-APIM: Preparing the Member Contrast

`dyadMLM` generates `.member_contrast_arbitrary` as an arbitrary `-1/+1`
member contrast. The fixed seed makes that assignment reproducible.
...
```{r data-exch-long-member-contrast, echo=FALSE, cache=TRUE, dependson='data-exch-long-apim-source'}
print_df(
  print_couple_preview_for_output(
    df_long_dim,
    couple_id = "31",
...
```{r fit-exch-long-apim-brms, results='hide', message=FALSE, warning=FALSE, cache=TRUE, dependson='data-exch-long-member-contrast'}
...
```{r fit-exch-long-dim-brms, results='hide', message=FALSE, warning=FALSE, cache=TRUE, dependson='data-exch-long-member-contrast'}
...
For appropriate exchangeable random effects, we can use the generated
`.member_contrast_arbitrary` column [@delrosarioPracticalGuideSpecifying2025; @kenny_ackerman_2023]:
...
For two partners coded with `.member_contrast_arbitrary = +1` and
`.member_contrast_arbitrary = -1`, the glmmTMB structure
`(1 | coupleID:diaryday) + (0 + .member_contrast_arbitrary | coupleID:diaryday)` uses
independent common and member-deviation components to imply a homogeneous
two-partner residual covariance.

This requires exactly two rows per couple-day and independent common and deviation
...
## Exchangeable Dyads - L-DIM/L-APIM: Results {.smaller}
...
print_df(
  ...
  rows_to_pack = list(
    'Fixed Effects' = c(1,6),
    'Random Effects' = c(7, 26),
    'Residual Structure' = c(27, 28)
    ),
  scroll_height = '50vh'
)
...
\operatorname{Var}(u_{\text{deviation intercept}})
$$
...
In the model output, the common-intercept variance is labelled `Intercept`, and
the deviation-intercept variance is labelled `.member_contrast_arbitrary`.
This rotation only holds when the common/couple-mean and member-deviation random
effects are uncorrelated. This is why we put them in separate random-effects blocks.
...
pkgs <- c(
  "brms", "easystats", "DHARMa", "tidyverse", "cmdstanr",
