...
In case of non-convergence, bad residuals or low predictive accuracy, the model is very likely miss-specified. 

You can try different families that match your data by using the `family()` argument. For instance, you can easily conduct an ordinal regression by simply setting `family = ordinal()`. For dichotomous outcomes you can use `family = bernoulli()`. Brms supports a variety of families and link functions for these families. In general, you should use the default link function (check `?brms::family.brmsfit()`). 
...
Note that sigma is reported on the log scale (see `?brms::family.brmsfit()`). Using `exp(value)` retrieves the simulated values well. 
...
df_long <- readRDS('Embed/df_long.rds')
...
Nlme model of this data for reference:
...
model1 <- lme(
  ...
  weights = varIdent(form = ~ 1 | gender), # heterogenious residual variances
  corr = corCompSymm(form = ~ 1 | coupleID/diaryday), # compound symmetry 
...
AR1 can changes the fixed effects quite a bit. You may always want to check which model 
performs better! Often the same day shock term and the time slopes take care of everything already. 
...
In this instance, the AR1 model is much worse! We keep the simpler model.

## Maximal Model Specification

We add all random slopes and their correlations. This is not converging in nlme or other
...
High Pareto-Ks can be a sign of overfitting or other problems. The loo values themselves cannot be interpreted individually, but we can compare models (next page)
...
The difference between the models seems quite small. No obvious overfitting, but also no improvement over the simpler model (even slightly worse, but hard to say for such small differences). 

We could follow our theory and the philosophy of the maximal random effects structure [@barrRandomEffectsStructure2013]. Alternatively, we could follow the philosophy of parsimony in this case. 

In case of overfitting, we could try to improve the model by subsequently excluding the smallest random effects (or correlations between them) and compare various models. For more guidance on random effects check out @delrosarioPracticalGuideSpecifying2025. 
...
One approach is to use the `report` package's approach to obtain a p-value (see their documentation): 
...
formula <- bf(
  satisfaction ~ 1 + 
    ...
    # we cannot distniguish each partners variance from noise. 
    # (1 | coupleID)
...
df_long_apim2 <- df_long_apim %>%
  dplyr::select('coupleID', 'userID', 'diaryday', 'diaryday_c', 'mvpa', 
...
priors <- c(
  prior(normal(50, 20), class = "Intercept"),
  prior(normal(0, 10/sqrt(2)), class = "b") # just to match prior in apim. any weakly informative prior is fine
)
