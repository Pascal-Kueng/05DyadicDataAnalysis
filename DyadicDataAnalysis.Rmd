...
Replacing `cosy()` with `(1 | coupleID:diaryday) + ar(...)` is possible, but it is not the same model. It changes the same-day dyadic dependency and adds an AR process at the same time, so a LOO comparison would not isolate the value of AR(1).

The cleanest solution would be a covariance structure that jointly models same-day partner interdependence and within-person autocorrelation, or a multivariate/custom Stan model.

## Distinguishable Dyads - L-APIM - Dynamic Workaround

As a sensitivity analysis, a dynamic MLM can account for temporal dependence by including lagged outcomes as predictors. In dyadic data, this can include actor inertia and partner spillover: today's closeness is predicted by yesterday's own closeness and yesterday's partner closeness, in addition to the concurrent APIM predictors.

This is not a drop-in replacement for residual AR(1). The APIM coefficients become effects conditional on prior closeness, not total within-person associations. Lagged-outcome models can also suffer from dynamic-panel bias, often called Nickell bias [@nickellBiasesDynamicModels1981], because the lagged outcome is related to person/couple effects and prior residuals. This is especially problematic with short time series; with longer intensive longitudinal panels it is often less severe, but it should still be discussed.

```{r eval = FALSE}
df_long_dyn <- df_long_apim |>
  dplyr::arrange(userID, diaryday) |>
  dplyr::group_by(userID) |>
  dplyr::mutate(closeness_actor_lag = dplyr::lag(closeness)) |>
  dplyr::ungroup() |>
  dplyr::group_by(coupleID, diaryday) |>
  dplyr::mutate(
    closeness_partner_lag = closeness_actor_lag[match(3 - gender, gender)]
  ) |>
  dplyr::ungroup() |>
  dplyr::filter(!is.na(closeness_actor_lag), !is.na(closeness_partner_lag)) |>
  dplyr::mutate(
    closeness_actor_lag_c = closeness_actor_lag - mean(closeness_actor_lag),
    closeness_partner_lag_c = closeness_partner_lag - mean(closeness_partner_lag)
  )
```
...
is_male:closeness_actor_lag_c +
is_male:closeness_partner_lag_c +
is_female:closeness_actor_lag_c +
is_female:closeness_partner_lag_c
```

Use this as a sensitivity analysis for temporal dynamics, not as the primary APIM unless the conditional-on-lag interpretation matches the research question.

## Maximal Model Specification (no AR(1))

We add all random slopes and their correlations. This is not converging in `nlme`. 

The same maximal random-effects idea can also be attempted in glmmTMB. This is a useful convergence check, but it estimates a large covariance matrix with only 100 couples, so inspect warnings, boundary estimates, and the Hessian before trusting the fit.

```{r eval = FALSE}
model_dist_apim_long_complex_glmmTMB <- glmmTMB(
  closeness ~ 
    0 + is_male + is_female +
    is_male:diaryday_c + is_female:diaryday_c +
    is_male:provided_support_actor_cbp +
...
# Combine estimates
est_tbl <- bind_rows(fix_labeled, sd_labeled, cosy_summ, resid_summ)
...
model1_glmmTMB <- glmmTMB(
  satisfaction ~ 1 +
    ...
    # we do not need to take gender here, we could construct arbitrary categorization. 
...
formula <- bf(
  satisfaction ~ 1 + 
    ...
    # sigma ~ 1 is implied and does not have to be specified. 
)
