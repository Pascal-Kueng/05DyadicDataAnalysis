...
However, for families that have no residuals, this does not work and we need to model the 
non-independence in the random effects. 

To have a single model specification that works for all model families, we can use this: 
...
formula_dist_apim_c <- bf(
  satisfaction ~ 
    ...
    # Modelling residual correlation.
    (0 + is_male + is_female | coupleID)
  
  # For non-Gaussian models, we omit the sigma argument.  
  # For Gaussion models, we can force sigma to a small value:
  , sigma = 1e-6
)
