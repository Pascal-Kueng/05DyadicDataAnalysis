...
## Readable Names in This Tutorial {.tutorial-alias-table}

`dyadMLM` returns explicit, self-documenting names. This tutorial drops the
generated `.dy_` prefix and shortens the `within_dyad_dev` suffix to
`dyad_dev`, `cwp_dev`, or `cbp_dev`.

**General and cross-sectional aliases**

| Canonical `dyadMLM` name | Tutorial alias | Interpretation |
|:---|:---|:---|
| `.dy_is_female_x_male_female` | `is_female` | female member of a female-male dyad |
| `.dy_is_female_x_male_male` | `is_male` | male member of a female-male dyad |
| `.dy_member_contrast_female_x_male_arbitrary` | `idiff` | female-male exchangeability contrast |
| `.dy_member_contrast_assumed_exchangeable_arbitrary` | `idiff` | role-free exchangeability contrast |
| `.dy_communication_actor` | `communication_actor` | actor's communication score |
| `.dy_communication_partner` | `communication_partner` | partner's communication score |
| `.dy_communication_dyad_mean_gmc` | `communication_dyad_mean_gmc` | grand-mean-centered dyad mean |
| `.dy_communication_within_dyad_dev` | `communication_dyad_dev` | member's within-dyad deviation |

## Applying the Tutorial Aliases

To keep these aliases consistent, this tutorial uses
`rename_dyadmlm_for_tutorial()`, defined in
...
```{r dyadmlm-tutorial-alias-example, eval=FALSE}
model_data <- prepared_data |>
  rename_dyadmlm_for_tutorial(
...
calling `dyadMLM` preparation functions.
...
For cross-sectional APIMs, `communication_actor` and
`communication_partner` contain the original communication scores. The earlier
version of this tutorial grand-mean centered both columns manually. At the
likelihood level, removing that centering changes neither slopes nor fitted
values (aside from prior impact in Bayesian fits), but the role-specific
intercepts now refer to `communication = 0` rather than average communication.
...
panel <- panel |>
  ...
  dplyr::mutate(
    ...
    # latent components
    cwp_actor   = sup_cwp,
    cwp_partner = sup_cwp_partner,
    cbp_actor   = mu_actor   - grand_mu,
    cbp_partner = mu_partner - grand_mu,

    # deterministic mean
    mu_it =
      fix_b0 + fix_time * diaryday +
      fix_bp_actor   * cbp_actor   + fix_bp_partner * cbp_partner +
      fix_wp_actor   * cwp_actor   + fix_wp_partner * cwp_partner +
      re_b0 + re_time * diaryday +
      re_wp_actor * cwp_actor + re_wp_part * cwp_partner +
      z_day
  )
...
```{r data-dist-long-apim-reshape, results='hide', warning=FALSE, message=FALSE, cache=TRUE, dependson='simulate-longitudinal-data'}
...
df_long_apim <- prepare_dyad_data(
  ...
  rename_dyadmlm_for_tutorial(
    # Keep dyadMLM's cwp_actor/cbp_partner component ordering.
    predictor = "provided_support"
  ) %>%
  mutate(
...
print_df(
  print_couple_preview_for_output(
    ...
    pdf_cols = c(
      ...
      "is_male", "is_female", "closeness", "provided_support_cwp_actor",
      "provided_support_cwp_partner", "provided_support_cbp_actor",
...
With repeated measures we need to center variables in order to not conflate levels of analysis [@bolgerIntensiveLongitudinalMethods2013]:
...
`prepare_dyad_data()` performs this predictor decomposition before creating
...
```{r fit-dist-apim-long-glmmtmb, results='hide', message=FALSE, warning=FALSE, cache=TRUE, dependson='data-dist-long-apim-reshape'}
...
```{r fit-dist-apim-long-simple-brms, results='hide', message=FALSE, warning=FALSE, cache=TRUE, dependson='data-dist-long-apim-reshape'}
#| code-line-numbers: "1-6|8-13|15-20|22-26|28-34|36-48|50-61"
...
## Distinguishable Dyads - L-APIM - AR(1)? {.smaller}

In this model, `unstr(time = gender, gr = coupleID:diaryday)` handles same-day
partner interdependence. Current univariate `brms` models cannot combine this
residual correlation structure with `ar()` in the same formula [@R-brms].

Standard `glmmTMB` can add a same-day dyadic block and independent
person-specific AR(1) processes:
...
This additive model represents same-day dyadic shocks plus person-specific
persistence. It splits lag-zero variance across the two blocks. These residual
blocks contribute zero cross-partner covariance across different days, although
dyad-level random effects can still induce cross-day covariance; this is not one
separable partner-by-time residual process [@R-glmmTMB].
...
```{r fit-dist-apim-long-complex-brms, results='hide', message=FALSE, warning=FALSE, cache=TRUE, dependson='data-dist-long-apim-reshape'}
#| classes: code-50vh
...
## Maximal Model: Diagnostics and Prediction Target {.smaller}

- Check convergence and model fit before comparing predictive accuracy.
- `loo()` holds out one observation while retaining its partner and the dyad's
  other days. This evaluates conditional observation-level prediction, not a
  completely new dyad.
- For Gaussian `unstr()` models, `brms` computes the required conditional
  density $p(y_i\mid y_{-i},\theta)$ for non-factorizable LOO
  [@buerknerEfficientLeaveOneOut2021].
- New-dyad prediction would instead require a grouped validation design
  [@loo2017a].

## Maximal Model: PSIS-LOO Diagnostics {.smaller}
...
Pareto $k$ diagnoses the importance-sampling approximation; high values are
not themselves evidence of overfitting [@loo2017a].

## Maximal Model: LOO Comparison {.smaller}
...
The complex model predicts the held-out observations better by 15.4 ELPD
units (SE 6.7). This supports its additional structure for this prediction
target, but does not prove that every variance or correlation is necessary;
ELPD differences should be interpreted with their uncertainty
[@loo2025a]. If performance were similar, theory, identifiability, and
parsimony should guide simplification
[@barrRandomEffectsStructure2013; @delrosarioPracticalGuideSpecifying2025].
...
```{r results-dist-long-ground-truth, echo = FALSE, message=FALSE, warning=FALSE, cache=TRUE, dependson=c('fit-dist-apim-long-complex-brms', 'simulate-longitudinal-data', 'data-dist-long-apim-reshape')}
...
true_re_tbl <- tibble::tibble(
  ...
  `is_male:provided_support_cwp_actor` = RE$re_wpA_m,
  `is_male:provided_support_cwp_partner` = RE$re_wpP_m,
  `is_female:provided_support_cwp_actor` = RE$re_wpA_f,
  `is_female:provided_support_cwp_partner` = RE$re_wpP_f
)
...
re_sd_labels <- c(
  ...
  "is_male:provided_support_cwp_actor" = "Actor WP slope SD (male)",
  "is_male:provided_support_cwp_partner" = "Partner WP slope SD (male)",
  "is_female:provided_support_cwp_actor" = "Actor WP slope SD (female)",
  "is_female:provided_support_cwp_partner" = "Partner WP slope SD (female)"
)
...
re_cor_labels <- c(
  ...
  "is_male:provided_support_cwp_actor" = "Actor WP slope (male)",
  "is_male:provided_support_cwp_partner" = "Partner WP slope (male)",
  "is_female:provided_support_cwp_actor" = "Actor WP slope (female)",
  "is_female:provided_support_cwp_partner" = "Partner WP slope (female)"
)
...
fixed_truth_tbl <- tibble::tribble(
  ...
  "Between-person", "Actor BP (male)", "is_male:provided_support_cbp_actor",
  "Between-person", "Partner BP (male)", "is_male:provided_support_cbp_partner",
  "Between-person", "Actor BP (female)", "is_female:provided_support_cbp_actor",
  "Between-person", "Partner BP (female)", "is_female:provided_support_cbp_partner",
  "Within-person", "Actor WP (male)", "is_male:provided_support_cwp_actor",
  "Within-person", "Partner WP (male)", "is_male:provided_support_cwp_partner",
  "Within-person", "Actor WP (female)", "is_female:provided_support_cwp_actor",
  "Within-person", "Partner WP (female)", "is_female:provided_support_cwp_partner"
) |>
...
map_fixed <- tibble::tribble(
  ...
  "^b_is_male:provided_support_cbp_actor$",  "Actor BP (male)",
  "^b_is_male:provided_support_cbp_partner$","Partner BP (male)",
  "^b_is_female:provided_support_cbp_actor$","Actor BP (female)",
  "^b_is_female:provided_support_cbp_partner$","Partner BP (female)",
  "^b_is_male:provided_support_cwp_actor$",  "Actor WP (male)",
  "^b_is_male:provided_support_cwp_partner$","Partner WP (male)",
  "^b_is_female:provided_support_cwp_actor$","Actor WP (female)",
  "^b_is_female:provided_support_cwp_partner$","Partner WP (female)"
)
...
![Exchangeable APIM](Embed/APIM_ind_3.png){width=72%}
[e.g., @delrosarioPracticalGuideSpecifying2025; @kenny_ackerman_2023]
...
For illustration, we now prepare the same female-male dyads under an
exchangeability constraint. This is a modelling assumption/specification, not a
claim that the data-generating dyads are truly exchangeable. Starting again from the
raw data makes that changed assumption explicit.

## Exchangeable Dyads - Cross-Sectional APIM: Preparation

```{r data-exch-prepare, results='hide', message=FALSE, warning=FALSE, cache=TRUE, dependson='simulate-cross-sectional-data'}
df_exchangeable <- prepare_dyad_data(
  data = df,
  dyad = coupleID,
  member = userID,
  role = gender,
...
print_apim_preview(df_exchangeable)
...
```{r fit-exch-apim-glmmtmb, results='hide', message=FALSE, warning=FALSE, cache=TRUE, dependson='data-exch-prepare'}
...
The raw `glmmTMB` summary reports the common and deviation variance
components. The homogeneous member variance and partner correlation are
derived from them; they are not printed directly:
...
## Exchangeable Dyads - Cross-Sectional APIM: Interpreting the Covariance {.smaller}

Let $\tau_c^2$ be the variance of the common intercept and $\tau_d^2$ the
variance of the independent `idiff` deviation. With `idiff = -1/+1`, the
...
$$
\operatorname{Cov}(e_1,e_2)=\tau_c^2-\tau_d^2,
\qquad
\rho=\frac{\tau_c^2-\tau_d^2}{\tau_c^2+\tau_d^2}.
$$

The model can therefore represent negative partner correlation when
$\tau_d^2>\tau_c^2$ [@delrosarioPracticalGuideSpecifying2025]. For repeated
measures, index the same structure at the couple-day level.
...
```{r fit-exch-apim-brms, results='hide', message=FALSE, warning=FALSE, cache=TRUE, dependson='data-exch-prepare'}
...
communication_grand_mean <- mean(df_exchangeable$communication)
apim_centered_intercept <-
  fixef(model_exch_apim)["Intercept", "Estimate"] +
  communication_grand_mean * (
    fixef(model_exch_apim)["communication_actor", "Estimate"] +
    fixef(model_exch_apim)["communication_partner", "Estimate"]
  )
...
## Exchangeable Dyads - APIM: LOO Prediction Target {.smaller}

Observation-level PSIS-LOO can compare these models because they fit the same
responses. With `unstr()`, `brms` uses the appropriate conditional predictive
density for the correlated Gaussian outcomes
[@buerknerEfficientLeaveOneOut2021]. Holding out one partner while the other
remains observed evaluates conditional prediction—not a formal test of
distinguishability or prediction of a new dyad.
...
## Exchangeable Dyads - APIM: LOO Comparison {.smaller}
...
For this observation-level target, the distinguishable model predicts better.
The ELPD difference is descriptive predictive evidence; `se_diff` summarizes
its uncertainty [@loo2017a; @loo2025a].
...
if (knitr::is_html_output()) {
  ...
  arrows(4.1, 6.7, 5.9, 5.6, length = 0.1)
  arrows(4.1, 3.3, 5.9, 4.4, length = 0.1)
...
The same `prepare_dyad_data()` call generated the DIM columns directly from
the raw long-format observations.
...
print_df(
  head(
    dplyr::select(
      df_exchangeable,
      userID, coupleID, satisfaction, communication,
      communication_dyad_mean_gmc, communication_dyad_dev, idiff
    )
  )
)
...
`communication_dyad_mean_gmc` is the grand-mean-centered dyad mean, while
`communication_dyad_dev` is each member's deviation from that dyad mean.
...
```{r fit-exch-dim-brms, results='hide', message=FALSE, warning=FALSE, cache=TRUE, dependson='data-exch-prepare'}
...
## Equivalence APIM and DIM {.smaller}

`dyadMLM` grand-mean centers the DIM dyad mean, whereas these APIM predictors
are raw. APIM and DIM slopes require the sum/difference transformations shown
later; their residual covariance is directly comparable. The printed intercepts
also use different origins, so shift the APIM intercept to average communication
before comparing it [@Bolger2025Unified]:

$$
b_{0,\,APIM(GMC)} = b_{0,\,APIM(raw)} + \bar{x}
(b_{actor} + b_{partner}).
$$
...
```{=html}
  ...
  <!-- Model coefficients -->
  <div id="model-coefs"
       data-apim-b0="`r if (knitr::is_html_output()) round(apim_centered_intercept, 2) else ''`"
       data-apim-ba="`r if (knitr::is_html_output()) round(fixef(model_exch_apim)['communication_actor','Estimate'], 2) else ''`"
       data-apim-bp="`r if (knitr::is_html_output()) round(fixef(model_exch_apim)['communication_partner','Estimate'], 2) else ''`"
       data-dim-b0 ="`r if (knitr::is_html_output()) round(fixef(model_exch_dim)['Intercept','Estimate'], 2) else ''`"
       data-dim-cb ="`r if (knitr::is_html_output()) round(fixef(model_exch_dim)['communication_dyad_mean_gmc','Estimate'], 2) else ''`"
       data-dim-cw ="`r if (knitr::is_html_output()) round(fixef(model_exch_dim)['communication_dyad_dev','Estimate'], 2) else ''`"></div>
...
```{=html}
  ...
  <div id="sd2-coefs"
       data-b0="`r if (knitr::is_html_output()) round(apim_centered_intercept, 2) else ''`"
       data-ba="`r if (knitr::is_html_output()) round(fixef(model_exch_apim)['communication_actor','Estimate'], 2) else ''`"
       data-bp="`r if (knitr::is_html_output()) round(fixef(model_exch_apim)['communication_partner','Estimate'], 2) else ''`"></div>
...
In the **distinguishable** case, the full **Dyadic Score Model (DSM)** [e.g., @stadlerHowCompanionshipRelated2023] is equivalent to the corresponding full APIM (aside from prior impact in Bayesian fits) [@Bolger2025Unified; @iidaModelsInterdependentIndividuals2018].



## Exchangeable Dyads - L-DIM/L-APIM: Simulated Data

We start again from the raw longitudinal data and omit `role`, which tells
`dyadMLM` to treat the two members as exchangeable. Requesting both model
types creates the APIM and DIM predictors in the same model-ready object.

```{r data-exch-long-apim-source, results='hide', cache=TRUE, dependson='simulate-longitudinal-data'}
...
print_df(
  print_couple_preview_for_output(
    ...
    couple_id = "31",
    pdf_cols = c(
      "userID", "coupleID", "idiff", "diaryday", "diaryday_c",
      "closeness", "provided_support_cwp_actor",
      "provided_support_cwp_partner", "provided_support_cbp_actor",
      "provided_support_cbp_partner"
    )
  ),
...
The generated `provided_support_cwp_dyad_mean` and `provided_support_cwp_dev`
columns form the within-person DIM. The corresponding
`provided_support_cbp_dyad_mean` and `provided_support_cbp_dev` columns form the
between-person DIM [@bolgerIntensiveLongitudinalMethods2013].

## Exchangeable Dyads - L-DIM/L-APIM: Preparing the Mean-Deviation `idiff` Approach

The `idiff` alias refers to `dyadMLM`'s generated arbitrary `-1/+1` member
contrast. The fixed seed makes that arbitrary assignment reproducible.

::: {.content-visible when-format="pdf"}

```{r data-exch-long-dim-preview, echo=FALSE, cache=TRUE, dependson='data-exch-long-apim-source', eval=knitr::is_latex_output()}
print_df(
  print_couple_preview_for_output(
    df_long_dim,
    couple_id = "31",
    pdf_cols = c(
      "userID", "coupleID", "idiff", "diaryday", "diaryday_c",
      "provided_support_cbp_dyad_mean", "provided_support_cbp_dev",
...
```{r data-exch-long-idiff, echo=FALSE, cache=TRUE, dependson='data-exch-long-apim-source'}
print_df(
  print_couple_preview_for_output(
    df_long_dim,
    couple_id = "31",
...
## Exchangeable Dyads - L-DIM/L-APIM: Common and `idiff` Components

For two partners coded with `idiff = +1` and `idiff = -1`, the glmmTMB
structure `(1 | coupleID:diaryday) + (0 + idiff | coupleID:diaryday)` uses
independent common and member-deviation components to imply a homogeneous
two-partner residual covariance.

This requires exactly two rows per couple-day, independent common and `idiff`
blocks, and `dispformula = ~ 0`. If their variances are $\tau_c^2$ and
$\tau_d^2$, member variance is $\tau_c^2+\tau_d^2$, covariance is
$\tau_c^2-\tau_d^2$, and their ratio is the correlation. No manual partner
indicators are needed [@delrosarioPracticalGuideSpecifying2025].
